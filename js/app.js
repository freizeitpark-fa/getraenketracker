...
function standaloneTripReportHtml(model = tripReportExportModel(), options = {}) {
  ...
  const autoPrint = options.autoPrint === true
    ...
    : '';
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="only light"><meta name="theme-color" content="#ffffff"><title>${esc(title)}</title><style>${reportDocumentCss()}</style></head><body>${tripReportDocumentBodyHtml(model)}${autoPrint}</body></html>`;
}
...
function printTripReport() {
  ...
  try {
    ...
    printWindow.document.write(standaloneTripReportHtml(model, { autoPrint: true }));
